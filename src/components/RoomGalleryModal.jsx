...
const RoomGalleryModal = ({ 
    ...
    return createPortal(
        <div className="fixed inset-0 z-[4000] flex flex-col lg:flex-row bg-black/95 backdrop-blur-xl animate-in fade-in duration-300">
            ...
            <div className="relative flex-1 flex flex-col h-[50vh] lg:h-full lg:p-6 overflow-hidden select-none" onClick={onClose}>
                ...
                >
                    ...
                    <div className="absolute top-0 left-0 right-0 p-4 lg:p-6 flex justify-between items-start z-20 pointer-events-none bg-gradient-to-b from-black/50 to-transparent">
                        <div className="bg-black/40 backdrop-blur-md px-4 py-2 rounded-[4px] text-white font-black tracking-widest text-xs uppercase border border-white/10 shadow-lg pointer-events-auto">
                            {currentIndex + 1} / {validImages.length} {tLocal('photos')}
                        </div>
                        {/* Mobile Close Button */}
                        <button
                            onClick={onClose}
                            className="lg:hidden size-10 rounded-[4px] bg-black/40 backdrop-blur-md text-white flex items-center justify-center border border-white/10 shadow-lg pointer-events-auto hover:bg-white/20 transition-colors"
                        >
                    ...
                    {validImages.length > 1 && (
                        <>
                            <button
                                ...
                                className="hidden lg:flex absolute left-6 size-12 rounded-[4px] bg-black/40 hover:bg-white/10 backdrop-blur-md text-white items-center justify-center transition-all group z-20 border border-white/10 shadow-xl"
                            >
                                <span className="material-symbols-outlined text-3xl group-hover:-translate-x-1 transition-transform">chevron_left</span>
                            </button>
                            <button
                                onClick={handleNext}
                                className="hidden lg:flex absolute right-6 size-12 rounded-[4px] bg-black/40 hover:bg-white/10 backdrop-blur-md text-white items-center justify-center transition-all group z-20 border border-white/10 shadow-xl"
                            >
                        ...
                        <div className="flex gap-3 overflow-x-auto no-scrollbar max-w-full pb-2 snap-x px-2">
                            {validImages.map((img, idx) => (
                                <button
                                    ...
                                    className={`relative size-16 lg:size-20 shrink-0 rounded-[4px] overflow-hidden transition-all duration-300 snap-center border-2 ${idx === currentIndex ? 'border-primary ring-2 ring-primary/30 scale-105' : 'border-transparent opacity-50 hover:opacity-100 hover:scale-95'}`}
                                >
            ...
            <div className="flex-1 lg:flex-none lg:w-[360px] xl:w-[420px] bg-white dark:bg-slate-900 rounded-[4px] flex flex-col h-[50vh] lg:h-full overflow-hidden shadow-[-10px_0_30px_rgba(0,0,0,0.5)] z-30 transition-colors duration-300">
                
                {/* Desktop Close Button */}
                <div className="hidden lg:flex justify-end p-6 pb-0">
                    <button
                        onClick={onClose}
                        className="size-12 rounded-[4px] bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 flex items-center justify-center transition-all duration-300"
                    >
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-5 lg:p-6 pt-8 lg:pt-4">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                        {squareMeter && (
                            <span className="flex items-center gap-1.5 bg-blue-500/10 text-blue-600 dark:text-blue-400 text-[10px] font-black px-2.5 py-1.5 rounded-[4px] uppercase tracking-wider border border-blue-500/10">
                                <span className="material-symbols-outlined text-sm">square_foot</span>
                                {squareMeter}
                            </span>
                        )}
                        <span className="flex items-center gap-1.5 bg-orange-500/10 text-orange-600 dark:text-orange-400 text-[10px] font-black px-2.5 py-1.5 rounded-[4px] uppercase tracking-wider border border-orange-500/10">
                            <span className="material-symbols-outlined text-sm">group</span>
                            {maxAdult + maxChildren} {tLocal('pax')}
                        </span>
                        {maxAdult > 0 && (
                            <span className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[10px] font-black px-2.5 py-1.5 rounded-[4px] uppercase tracking-wider">
                    ...
                    {description && (
                        <div className="mb-6">
                            ...
                            <div className="bg-slate-50/80 dark:bg-slate-800/40 p-4 rounded-[4px] border border-slate-100 dark:border-slate-800">
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                {processedAttributes.map((attr, idx) => (
                                    <div key={idx} className="flex items-center gap-3 p-3 rounded-[4px] bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md hover:border-primary/30 transition-all group">
                                        <div className="size-8 rounded-[4px] bg-primary/10 text-primary flex items-center justify-center shrink-0 group-hover:bg-primary group-hover:text-white transition-colors">
